...
export default function TeacherDashboard() {
  ...
  const chartData = useMemo(() => {
    return uniqueDatesArray.map(date => {
      const attsOnDate = attendances.filter((a: any) => (a.attributes?.date || a.date) === date);
      const presentOnDate = attsOnDate.filter((a: any) => (a.attributes?.currentStatus || a.currentStatus) === 'present').length;
      const rate = Math.round((presentOnDate / Math.max(students.length, 1)) * 100);
      return {
  ...
  }, [uniqueDatesArray, attendances, students.length]);
  ...
  const studentStats = useMemo(() => {
    return students.map((stu: any) => {
      ...
      let dateStatus = 'absent';
      let dateType = 'auto';
      if (selectedDate !== 'all') {
        const attForDate = stuAttendances.find((a: any) => (a.attributes?.date || a.date) === selectedDate);
        if (attForDate) {//@ts-ignore
          dateStatus = attForDate.attributes?.currentStatus || attForDate.currentStatus;
          //@ts-ignore
          dateType = attForDate.attributes?.type || attForDate.type;
        }
      }
  ...
  }, [students, attendances, searchQuery, selectedDate, uniqueDatesArray]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-1 pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full no-print">
        ...
        {/* Analytics Chart */}
        {chartData.length > 0 && selectedDate === 'all' && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12 bg-background border border-foreground/10 rounded-[3rem] p-8 shadow-sm">
            <h3 className="text-xl font-black text-foreground mb-6 pl-2">Attendance Trend Analytics</h3>
              ...
              <thead>
                <tr className="border-b border-foreground/5 bg-foreground/[0.01]">
                  ...
                  {selectedDate === 'all' ? (
                    ...
                    <th className="px-8 py-6 text-left text-[10px] font-black uppercase text-foreground/30 tracking-widest text-center">Daily Status</th>
                  )}
                        ...
                        <td className="px-8 py-5 text-center">
                          {student.dateStatus === 'present' ? (
                            <span className="inline-flex items-center gap-1.5 text-xs font-black text-emerald-600 uppercase border border-emerald-600/20 bg-emerald-500/10 px-4 py-2 rounded-full">
                              ...
                              {student.dateType === 'manual' && <span className="text-[9px] opacity-60 ml-1">(Manual)</span>}
                            </span>
                          ) : (
